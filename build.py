...
def adviser_card(person):
    body=f'<h3>{escape(person["name"])}</h3><p class="adviser-role">{escape(person["role"])}</p><p class="affiliation">{escape(person["note"])}</p><p>{escape(person["research"])}</p>'
    body+='<div class="profile-links">'+link(person['homepage'],'Homepage')+link(person['institute_profile'],'Institute profile')+link(person['institute_url'],'Institute')+'</div>'
    return f'<article class="person adviser"><span class="initials" aria-hidden="true">{escape(person["initials"])}</span><div>{body}</div></article>'
...
people+='<section class="advisers-section"><span class="eyebrow">ACADEMIC LINEAGE</span><h2>Academic advisers</h2><p class="adviser-acknowledgment">Prof. T. A. Abinandanan and Prof. Long-Qing Chen gave Prof. Saswata Bhattacharya the foundation on which his academic life was built. Without their teaching, trust and example, he would not be the researcher and teacher he is today.</p><div class="people advisers">'+''.join(adviser_card(p) for p in data['advisers'])+'</div></section>'
